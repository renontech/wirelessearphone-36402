  ...
  describe 'レビュー投稿機能' do
    ...
    context '投稿できない場合' do
      ...
      it 'pointが空だと投稿出来ない' do
        @review.point = ''
        @review.valid?
        expect(@review.errors.full_messages).to include("点数を入力してください")
      end
      it 'pointが半角数値でなければ投稿出来ない' do
        @review.point = '１０'
        @review.valid?
        expect(@review.errors.full_messages).to include("点数は数値で入力してください")
      end
      it 'pointに小数点が含まれていると投稿出来ない' do
        @review.point = 2.5
        @review.valid?
        expect(@review.errors.full_messages).to include("点数は整数で入力してください")
      end
      it 'pointが1未満の半角数値だと投稿できない' do
        @review.point = -1
        @review.valid?
        expect(@review.errors.full_messages).to include("点数は0以上の値にしてください")
      end
      it 'pointが5を超える半角数値だと投稿できない' do
        @review.point = 6
        @review.valid?
        expect(@review.errors.full_messages).to include("点数は5以下の値にしてください")
      end
